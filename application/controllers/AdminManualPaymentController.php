...
class AdminManualPaymentController extends CI_Controller{
	...
	public function index(){
        $searchView = $this->load->view("admin/manual-payment/search",[],TRUE);
      
        $this->LTemplate->headerAdmin();

        $this->load->view("admin/manual-payment",[
            "search" => $searchView,            
    ...
    public function searchPagi($pageQuery){  
        ...
        if(!empty($this->input->get('status'))){
            $pageQuery = $pageQuery->where($this->MManualPayment->table.'.status',$this->input->get('status'));
        }
