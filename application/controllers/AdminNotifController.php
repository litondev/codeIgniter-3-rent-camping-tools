...
class AdminNotifController extends CI_Controller{
	...
	public function index(){
        $searchView = $this->load->view("admin/notif-admin/search",[],TRUE);

        $this->LTemplate->headerAdmin();

        $this->load->view("admin/notif-admin",[
            "search" => $searchView,
            "notifAdmin" => $this->getPagiData(),
            "notifAdmin_link" => $this->getPagiLink()
        ]);
    ...
    public function getPagiData(){       
        ...
        $pageQuery = $pageQuery->get($this->MNotifAdmin->table,10,$page)->result_array();                   
    ...
    public function getPagiLink(){
        ...
        $pagiQueryCount = $pagiQueryCount->count_all_results($this->MNotifAdmin->table);       
    ...
    public function searchPagi($pageQuery){  
        ...
        if(!empty($this->input->get('content'))){
            $pageQuery = $pageQuery->like("content",$this->input->get('content'));
        }
